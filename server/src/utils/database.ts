...
import { AssetFileType, AssetVisibility, DatabaseExtension, ExifOrientation, SharingPermission } from 'src/enum';
...
export function withFacesAndPeople(
  ...
  { withHidden, withDeletedFace, userId }: { withHidden?: boolean; withDeletedFace?: boolean; userId?: string } = {},
) {
  return jsonArrayFrom(
    eb
      .selectFrom('asset_face')
      .select((eb) =>
        jsonObjectFrom(
          eb
            .selectFrom('person')
            .selectAll('person')
            .whereRef('person.groupId', '=', (eb) =>
              eb.selectFrom('person').select('person.groupId').whereRef('person.id', '=', 'asset_face.personId'),
            )
            .$if(!!userId, (qb) => qb.orderBy((eb) => eb('person.ownerId', '=', userId!), 'desc'))
            .limit(1),
        ).as('person'),
      )
      .selectAll('asset_face')
